...
function runRouteLifecycle(router, previous, matches, isCurrent) {
  ...
  for (const match of previous) {
    if ((isCurrent == null ? void 0 : isCurrent()) === false) return;
...
async function contextualize(router, lane, options, end, planSuccessfulLane, retainedEnd) {
  ...
  for (let index = options[7] ?? 0; index < end; index++) {
    ...
    (_b = options[8]) == null ? void 0 : _b.call(options);
    try {
...
function transferMatchResources(router, previous, next, deferSameIdFlight) {
  ...
  for (const match of previous) if (!(next == null ? void 0 : next.includes(match))) {
    const flight = match._flight;
    match._flight = void 0;
    if (deferSameIdFlight && (flight == null ? void 0 : flight[2]) === 1 && ((_a = router._flights) == null ? void 0 : _a.get(match.id)) === flight && !((_b = router._tx) == null ? void 0 : _b[6]) && (next == null ? void 0 : next.some((candidate) => candidate.id === match.id))) flight[2] = 0;
    else {
...
function createLoaderTask(router, lane, index, tasks, semanticParent, options, retainedEnd) {
  ...
  const background = !!(routeLoader && reload && match.status === "success" && !preload && !options[5] && ((typeof routeLoader === "function" ? void 0 : routeLoader == null ? void 0 : routeLoader.staleReloadMode) ?? router.options.defaultStaleReloadMode) !== "blocking");
  ...
  if (blocking) {
    ...
    (_d = options[8]) == null ? void 0 : _d.call(options);
  }
  ...
  const outcome = (reloadFailure ? Promise.resolve(reloadFailure) : !blocking ? Promise.resolve([SUCCESS, match.loaderData]) : loadResource(router, lane, match, route, loader, semanticParent, preload, options[0])).then((result) => {
    ...
    if (blocking) {
      ...
      if (result[0] === SUCCESS) {
        if (routeLoader && !options[0].signal.aborted && !(!preload && ((_a2 = router._tx) == null ? void 0 : _a2[6]))) cacheLoaderMatch(router, match, plannedCacheMatch);
  ...
  const chunkFailure = waitFor(Promise.resolve().then(() => loadRouteChunk(route, void 0, onLazyReady)), options[0].signal).then(() => void 0, (cause) => [index, normalizeLaneError(route, cause, options)]).then((failure) => outcome.then((result) => {
    ...
    if (blocking && !failure && result[0] === SUCCESS && match.status === "pending" && options[2]()) {
      match.status = "success";
      (_a2 = options[8]) == null ? void 0 : _a2.call(options);
    }
    return failure;
  }));
  tasks.push([
...
function offerPending(router, tx) {
  ...
  let tookOver = false;
  const sessionMatchId = (_a = session == null ? void 0 : session[0][3][session[1]]) == null ? void 0 : _a.id;
  if ((session == null ? void 0 : session[0]) !== tx) if (session && ((_b = tx[3][session[1]]) == null ? void 0 : _b.id) === sessionMatchId) {
    session[0] = tx;
    tookOver = true;
  } else {
  ...
  for (let index = 0; index < matches.length; index++) {
    ...
    presentedPending = ((_c = presented[index]) == null ? void 0 : _c.id) === match.id && ((_d = presented[index]) == null ? void 0 : _d.status) === "pending";
...
function finishPending(router, tx) {
  ...
  if ((session == null ? void 0 : session[0]) === tx) {
    clearTimeout(session[3]);
...
function rollbackPublication(router, tx, lane, checkpoint) {
  ...
  checkpoint.published = false;
  for (const match of discarded) if (!restored.includes(match) && match._flight && ((_a = router._flights) == null ? void 0 : _a.get(match.id)) === match._flight) router._flights.delete(match.id);
  ...
  if (router._tx === tx && router._commitPromise === checkpoint.commitPromise) {
    (_b = router._commitPromise) == null ? void 0 : _b.resolve();
...
async function transitionRefresh(router, tx, lane, changeInfo) {
  ...
  const rollback = () => {
    ...
    (_a = router._cancelTransition) == null ? void 0 : _a.call(router);
    return restored;
  };
  try {
...
async function runClientTransaction(router, tx, forceStaleReload, onReady, sync, resolvedPrefix) {
  ...
  if ((pending == null ? void 0 : pending[0]) === tx) {
...
async function loadClientRoute(router, opts) {
  ...
  if (true) {
    (_c = (_b = (_a = router._tx) == null ? void 0 : _a[6]) == null ? void 0 : _b[2]) == null ? void 0 : _c.call(_b);
    rematerialize = !!router._refreshNextLoad || !!((_d = router._tx) == null ? void 0 : _d[6]);
  }
  ...
  try {
    ...
    if (!isRedirect(cause)) {
      ...
      (_e = router._commitPromise) == null ? void 0 : _e.resolve();
      router._commitPromise = void 0;
      return;
    }
    await router.navigate({
  ...
  const tx = [
    ...
    matches,
    Date.now(),
    Promise.resolve().then(() => runClientTransaction(router, tx, sameHref, () => offerPending(router, tx), opts == null ? void 0 : opts.sync, resolvedPrefix)).catch(() => {
...
async function refreshClientRoute(router) {
  ...
  (_d = router._flights) == null ? void 0 : _d.clear();
  router.clearCache();
...
function createLoaderTask2(router, lane, index, tasks, signal) {
  ...
  if (match.ssr === false) outcome = Promise.resolve([SKIPPED]);
    ...
    if (!loader) outcome = Promise.resolve([SUCCESS2, void 0]);
      ...
      if (result[0] !== REDIRECTED2 && ((signal == null ? void 0 : signal.aborted) || match.abortController.signal.reason === lane)) return [SKIPPED];
...
function CatchBoundary(props) {
  return (0, import_jsx_runtime3.jsx)(CatchBoundaryImpl, { ...props });
}
var CatchBoundaryImpl = class extends React$14.Component {
  constructor(..._args) {
    super(..._args);
